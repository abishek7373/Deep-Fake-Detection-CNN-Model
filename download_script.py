...
def download_file(url, out_file, report_progress=False):
    ...
    if os.path.isfile(out_file):
        existing_file_size = os.path.getsize(out_file)
    ...
    while retry_attempts > 0:
        try:
            # Request file with the appropriate headers
            req = urllib.request.Request(url, headers=headers)
            with urllib.request.urlopen(req) as response, open(out_file, 'ab') as out_file_handle:
                total_size = int(response.info().get('Content-Length').strip()) + existing_file_size
                block_size = 1024 * 8  # 8 KB
                with tqdm(total=total_size, initial=existing_file_size, unit='B', unit_scale=True, desc=url.split('/')[-1]) as pbar:
                    while True:
            ...
            if e.code == 416:
                print(f"HTTP Error 416: Requested Range Not Satisfiable. Trying to download the entire file.")
                headers = {}  # Reset headers to download the entire file
            else:
                print(f"HTTP Error {e.code}: {e.reason}. Retrying...")
            retry_attempts -= 1
            time.sleep(5)  # Wait before retrying
        except urllib.error.ContentTooShortError:
            print(f"WARNING: Download of {out_file} was interrupted. Trying to resume...")
            retry_attempts -= 1
            time.sleep(5)  # Wait before retrying
